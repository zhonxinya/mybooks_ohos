...
napi_value InitCore(napi_env env, napi_callback_info info) {
    ...
    bool ok = mybooks::CoreEngine::instance().init(filesDir, prefsDir);
    napi_value result;
...
ASYNC_API(TalebookGet, [](const std::string &input) {
    ...
    return mybooks::ApiService::instance().talebookGet(path);
})

ASYNC_API(TalebookPost, [](const std::string &input) {
    size_t sep = input.find('|');
    if (sep == std::string::npos) return mybooks::ApiService::instance().wrapError("invalid input");
    std::string path = input.substr(0, sep);
    std::string body = input.substr(sep + 1);
    return mybooks::ApiService::instance().talebookPost(path, body);
})
...
ASYNC_API(TalebookPostForm, [](const std::string &input) {
    ...
    std::string path = input.substr(0, sep);
    std::string body = input.substr(sep + 1);
    return mybooks::ApiService::instance().talebookPostForm(path, body);
})

ASYNC_API(TalebookDelete, [](const std::string &input) {
    return mybooks::ApiService::instance().talebookDelete(input);
})
...
ASYNC_API(TalebookDeleteWithBody, [](const std::string &input) {
    ...
    std::string path = input.substr(0, sep);
    std::string body = input.substr(sep + 1);
    return mybooks::ApiService::instance().talebookDeleteWithBody(path, body);
})

ASYNC_API(TalebookPatch, [](const std::string &input) {
    size_t sep = input.find('|');
    if (sep == std::string::npos) return mybooks::ApiService::instance().wrapError("invalid input");
    std::string path = input.substr(0, sep);
    std::string body = input.substr(sep + 1);
    return mybooks::ApiService::instance().talebookPatch(path, body);
})

ASYNC_API(TalebookPut, [](const std::string &input) {
    size_t sep = input.find('|');
    if (sep == std::string::npos) return mybooks::ApiService::instance().wrapError("invalid input");
    std::string path = input.substr(0, sep);
    std::string body = input.substr(sep + 1);
    return mybooks::ApiService::instance().talebookPut(path, body);
})
...
ASYNC_API(TalebookUploadFile, [](const std::string &input) {
    ...
    if (first == std::string::npos || second == std::string::npos || third == std::string::npos) {
        return mybooks::ApiService::instance().wrapError("invalid input");
    }
    ...
    return mybooks::ApiService::instance().talebookUploadFile(
...
ASYNC_API(TalebookUploadBook, [](const std::string &input) {
    size_t first = input.find('|');
    if (first == std::string::npos) return mybooks::ApiService::instance().wrapError("invalid input");
    size_t second = input.find('|', first + 1);
    if (second == std::string::npos) return mybooks::ApiService::instance().wrapError("invalid input");
    return mybooks::ApiService::instance().talebookUploadBook(
...
ASYNC_API(TalebookUploadBookBatch, [](const std::string &input) {
    return mybooks::ApiService::instance().talebookUploadBookBatch(input);
})
ASYNC_API(TalebookUploadFiles, [](const std::string &input) {
    ...
    if (second == std::string::npos) {
        return mybooks::ApiService::instance().talebookUploadFiles(path, input.substr(first + 1), "");
    }
    return mybooks::ApiService::instance().talebookUploadFiles(
        path,
        input.substr(first + 1, second - first - 1),
        input.substr(second + 1));
})
ASYNC_API(GetDownloadRecords, [](const std::string &) {
    return mybooks::ApiService::instance().getDownloadRecords();
})

ASYNC_API(AddDownloadRecord, [](const std::string &input) {
    return mybooks::ApiService::instance().addDownloadRecord(input);
})

ASYNC_API(DeleteDownloadRecord, [](const std::string &input) {
    return mybooks::ApiService::instance().deleteDownloadRecord(input);
})

ASYNC_API(GetReadingHistory, [](const std::string &) {
    return mybooks::ApiService::instance().getReadingHistory();
})

ASYNC_API(RecordReadingHistory, [](const std::string &input) {
    return mybooks::ApiService::instance().recordReadingHistory(input);
})

ASYNC_API(GetReaderConfig, [](const std::string &) {
    return mybooks::ApiService::instance().getReaderConfig();
})

ASYNC_API(SaveReaderConfig, [](const std::string &input) {
    return mybooks::ApiService::instance().saveReaderConfig(input);
})

ASYNC_API(DownloadBookLocal, [](const std::string &input) {
    return mybooks::ApiService::instance().downloadBookLocal(input, nullptr);
})

ASYNC_API(SecureSet, [](const std::string &input) {
    size_t sep = input.find('|');
    if (sep == std::string::npos) return mybooks::ApiService::instance().wrapError("invalid");
    mybooks::Database::instance().secureSet(input.substr(0, sep), input.substr(sep + 1));
    return mybooks::ApiService::instance().wrapOk("true");
})

ASYNC_API(SecureGet, [](const std::string &input) {
    auto val = mybooks::Database::instance().secureGet(input);
    return mybooks::ApiService::instance().wrapOk(val ? ("\"" + *val + "\"") : "null");
})

ASYNC_API(KvSet, [](const std::string &input) {
    size_t sep = input.find('|');
    if (sep == std::string::npos) return mybooks::ApiService::instance().wrapError("invalid");
    mybooks::Database::instance().kvSet(input.substr(0, sep), input.substr(sep + 1));
    return mybooks::ApiService::instance().wrapOk("true");
})

ASYNC_API(KvGet, [](const std::string &input) {
    auto val = mybooks::Database::instance().kvGet(input);
    return mybooks::ApiService::instance().wrapOk(val ? ("\"" + *val + "\"") : "null");
})

ASYNC_API(SonovelGet, [](const std::string &input) {
    return mybooks::ApiService::instance().sonovelGet(input);
})

ASYNC_API(AdminTrashBooks, [](const std::string &) {
    return mybooks::ApiService::instance().adminTrashBooks();
})

ASYNC_API(AdminTrashSize, [](const std::string &) {
    return mybooks::ApiService::instance().adminTrashSize();
})

ASYNC_API(AdminTrashRestore, [](const std::string &input) {
    return mybooks::ApiService::instance().adminTrashRestore(input);
})

ASYNC_API(AdminTrashPurge, [](const std::string &input) {
    return mybooks::ApiService::instance().adminTrashPurge(input);
})

ASYNC_API(AdminTrashClear, [](const std::string &) {
    return mybooks::ApiService::instance().adminTrashClear();
})
...
ASYNC_API(AdminBookReviews, [](const std::string &input) {
    ...
    size_t second = input.find('|', first + 1);
    if (second == std::string::npos) return mybooks::ApiService::instance().wrapError("invalid input");
    return mybooks::ApiService::instance().adminBookReviews(
        input.substr(0, first),
...
ASYNC_API(AdminBookReviewAction, [](const std::string &input) {
    size_t sep = input.find('|');
    if (sep == std::string::npos) return mybooks::ApiService::instance().wrapError("invalid input");
    return mybooks::ApiService::instance().adminBookReviewAction(input.substr(0, sep),
                                                                  input.substr(sep + 1));
})
...
ASYNC_API(AdminImportList, [](const std::string &input) {
    ...
    size_t second = input.find('|', first + 1);
    if (second == std::string::npos) return mybooks::ApiService::instance().wrapError("invalid input");
    return mybooks::ApiService::instance().adminImportList(
...
ASYNC_API(AdminImportRun, [](const std::string &input) {
    size_t sep = input.find('|');
    if (sep == std::string::npos) return mybooks::ApiService::instance().wrapError("invalid input");
    return mybooks::ApiService::instance().adminImportRun(input.substr(0, sep),
                                                           input.substr(sep + 1) == "1");
})

ASYNC_API(AdminImportCancel, [](const std::string &) {
    return mybooks::ApiService::instance().adminImportCancel();
})

ASYNC_API(AdminImportDelete, [](const std::string &input) {
    return mybooks::ApiService::instance().adminImportDelete(input);
})

ASYNC_API(AdminSyslog, [](const std::string &) {
    return mybooks::ApiService::instance().adminSyslog();
})

ASYNC_API(AdminResources, [](const std::string &) {
    return mybooks::ApiService::instance().adminResources();
})

ASYNC_API(AdminToolList, [](const std::string &) {
    return mybooks::ApiService::instance().adminToolList();
})

ASYNC_API(AdminToolAction, [](const std::string &input) {
    size_t sep = input.find('|');
    if (sep == std::string::npos) return mybooks::ApiService::instance().wrapError("invalid input");
    return mybooks::ApiService::instance().adminToolAction(input.substr(0, sep),
                                                             input.substr(sep + 1));
})
...
ASYNC_API(AdminMemoList, [](const std::string &input) {
    size_t sep = input.find('|');
    if (sep == std::string::npos) return mybooks::ApiService::instance().wrapError("invalid input");
    return mybooks::ApiService::instance().adminMemoList(
...
ASYNC_API(AdminMemoAction, [](const std::string &input) {
    ...
    size_t second = input.find('|', first + 1);
    if (second == std::string::npos) return mybooks::ApiService::instance().wrapError("invalid input");
    return mybooks::ApiService::instance().adminMemoAction(input.substr(0, first),
                                                             input.substr(first + 1, second - first - 1),
                                                             input.substr(second + 1));
})

ASYNC_API(AdminMemoDelete, [](const std::string &input) {
    return mybooks::ApiService::instance().adminMemoDelete(input);
})
...
static napi_value SetBaseUrl(napi_env env, napi_callback_info info) {
    ...
    mybooks::ApiService::instance().setBaseUrl(input.substr(0, sep), input.substr(sep + 1));
...
static napi_value SetSslVerify(napi_env env, napi_callback_info info) {
    ...
    mybooks::HttpClient::instance().setSslVerify(verify);
    napi_value undefined = nullptr;
    napi_get_undefined(env, &undefined);
    return undefined;
}
...
static napi_value Init(napi_env env, napi_value exports) {
    napi_property_descriptor desc[] = {
        ...
        {"talebookPostForm", nullptr, TalebookPostForm, nullptr, nullptr, nullptr, napi_default,
         nullptr},
        {"talebookDelete", nullptr, TalebookDelete, nullptr, nullptr, nullptr, napi_default, nullptr},
        {"talebookDeleteWithBody", nullptr, TalebookDeleteWithBody, nullptr, nullptr, nullptr,
         napi_default, nullptr},
        {"talebookPatch", nullptr, TalebookPatch, nullptr, nullptr, nullptr, napi_default, nullptr},
        {"talebookPut", nullptr, TalebookPut, nullptr, nullptr, nullptr, napi_default, nullptr},
...
static napi_module mybooksModule = {
    .nm_version = 1,
    .nm_flags = 0,
    .nm_filename = nullptr,
    .nm_register_func = Init,
    .nm_modname = "mybooks_core",
    .nm_priv = nullptr,
    .reserved = {0},
};

extern "C" __attribute__((constructor)) void RegisterMybooksModule(void) {
    napi_module_register(&mybooksModule);
}
